...
class_name EncounterModeGrid

var label_prefab: PackedScene = preload("res://scenes/grid_label.tscn")
var grid_texture_button_prefab: PackedScene = preload("res://scenes/encounter_mode/encounter_grid_texture_button.tscn")
var shields_vbox_prefab: PackedScene = preload("res://scenes/encounter_mode/encounter_shields_v_box.tscn")
...
var grid_container_texture_buttons: Array[EncounterGridTextureButton]
var shields_interfaces: Array[EncounterShieldsVBox]
var selected_grid: int
var hovered_grids: Array[int]

func _ready() -> void:
	populate_grid()
	encounter_signal_bus.redraw_grid.connect(draw_grid)
	encounter_signal_bus.combatant_switched.connect(draw_grid)
	encounter_signal_bus.encounter_loaded.connect(draw_grid)
...
func populate_grid() -> void:
	...
	for i in range(1,7):
		...
		grid_grid_container.add_child(l)
		for n in range(6):
			var b: EncounterGridTextureButton = grid_texture_button_prefab.instantiate()
			...
			b.encounter_data_manager = encounter_data_manager
...
func draw_grid() -> void:
	...
	var f: Frame = ResourceManager.frame_dict[encounter_data_manager.current_combatant.current_frame_build.frame_id]
	...
	for pi in encounter_data_manager.current_combatant.current_frame_build.frame_build_configuration:
		...
		if part.tags.has("reactor"):
			...
			while cells_to_check.size() > 0:
				for cell in cells_to_check:
					for p_inst in encounter_data_manager.current_combatant.current_frame_build.frame_build_configuration:
...
func create_shields_interfaces() -> void:
	var old_shields: Array[int] = encounter_data_manager.current_combatant.current_shields.duplicate()
	encounter_data_manager.current_combatant.current_shields.clear()
	for si in shields_interfaces:
		si.queue_free()
	shields_interfaces.clear()
	var frame: Frame = ResourceManager.frame_dict[encounter_data_manager.current_combatant.current_frame_build.frame_id]
	...
	for pi in encounter_data_manager.current_combatant.current_frame_build.frame_build_configuration:
		...
		if part.part_type == Constants.PartType.PartShield and part is Shield:
			var svb: EncounterShieldsVBox = shields_vbox_prefab.instantiate()
			svb.encounter_data_manager = encounter_data_manager
			...
			if old_shields.size() > iter:
				encounter_data_manager.current_combatant.current_shields.append(old_shields[iter])
			else:
				encounter_data_manager.current_combatant.current_shields.append(shield_capacity)
...
func populate_part_labels() -> void:
	...
	for p in encounter_data_manager.current_combatant.current_frame_build.frame_build_configuration:
...
func apply_damage() -> void:
	var c: Character = encounter_data_manager.current_combatant
	for gb in grid_container_texture_buttons:
		gb.clear_damage()
	for i in range(c.current_damage.size()):
		for n in range(c.current_damage[i]):
